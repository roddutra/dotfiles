...
The `pnpm/` folder is **reference-only** (not a stow target). It contains a hardened config for pnpm v10+ that mitigates compromised npm package attacks. Full rationale and per-setting explanation lives in [`pnpm/README.md`](./pnpm/README.md).

Two layers, both needed:

- **Global config** on this Mac, applied via `pnpm config set ... --global`. Protects every project you touch.
- **Per-project config** committed to each repo's `pnpm-workspace.yaml`. Protects teammates and CI.

### On a fresh machine

Apply the global settings once:
...
```shell
pnpm config get minimumReleaseAge        # should print 10080
pnpm config get dangerouslyAllowAllBuilds # should print false
pnpm config get blockExoticSubdeps        # should print true
```

The settings land in `~/Library/Preferences/pnpm/rc`. A non-secret snapshot lives at [`pnpm/global-rc.example`](./pnpm/global-rc.example) for reference (the real rc file is not stowed because it also stores your npm auth token).

### Applying to a project

Open the template at [`pnpm/pnpm-workspace.yaml.template`](./pnpm/pnpm-workspace.yaml.template) and copy the keys into the project's `pnpm-workspace.yaml` (create the file if it doesn't exist). Commit it so teammates and CI get the same protection.

Minimum keys to merge in:
...
Common follow-ups after installing in a new project:

- **Build script warning** (`Ignored build scripts: esbuild, sharp, ...`): run `pnpm approve-builds` inside the project to add legitimate native builds to `onlyBuiltDependencies`.
- **"Couldn't find package X in version Y"**: the version is less than 7 days old. Either wait, install once with `pnpm add <pkg> --allow-non-published-version`, or add the package to `minimumReleaseAgeExclude` if you need it regularly.

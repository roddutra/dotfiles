...
Use the target repository's test runner and instructions. Do not edit source
or tests while a test run is using the same checkout.

1. Run the smallest owner and sibling tests that cover the retained contract.
2. For removed source greps or plan assertions, run the executable script or
   dry-run that owns the real contract.
3. Run targeted formatting or linting and `git diff --check`.
4. Run any additional changed-file gate required by the target repository.
...
Commit, push, open a PR, or land only when authorized. Follow the target
repository's PR instructions. Land one coherent PR at a time; after landing,

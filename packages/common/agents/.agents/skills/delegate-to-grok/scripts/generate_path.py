#!/usr/bin/env python3
"""Generate correctly formatted file paths for a Grok delegation round.

Files live in the session directory alongside session.json, with simple
names: r1-brief.md, r1-report.md, r1-changes.md, r2-brief.md, etc.

Directory structure:
  ~/.grok-delegate/<project>/<YYYY-MM-DD>/<HHMMSS-title>/
    session.json
    r1-brief.md
    r1-report.md
"""
...
DELEGATE_DIR = Path.home() / ".grok-delegate"
...
def resolve_git_project_name(start_dir: Path | None = None) -> tuple[bool, str | None]:
    ...
    Grok reviews the files in the checkout you launched from). The chosen path
...
def validate_session_path(session_path: Path) -> None:
    """Ensure session file lives under DELEGATE_DIR and is named session.json."""
    resolved = session_path.resolve()
    reviews_resolved = DELEGATE_DIR.resolve()
    if not resolved.is_relative_to(reviews_resolved):
        print(f"Error: Session file must be under {DELEGATE_DIR}, got: {session_path}", file=sys.stderr)
        sys.exit(1)
...
def generate_paths(session_path: Path, round_num: int) -> dict:
    """Generate brief/report/changes/stream paths for a delegation round.
    ...
    Returns:
        Dict with brief_path, report_path, changes_path, stream_path
    """
    ...
    return {
        "brief_path": str(session_dir / f"r{round_num}-brief.md"),
        "report_path": str(session_dir / f"r{round_num}-report.md"),
        "changes_path": str(session_dir / f"r{round_num}-changes.md"),
        "stream_path": str(session_dir / f"r{round_num}-stream.jsonl"),
    }


def main():
    parser = argparse.ArgumentParser(description="Generate Grok delegation file paths for a round")

#!/usr/bin/env python3
"""List and filter past Grok delegation sessions.

Walks the directory tree under ~/.grok-delegate/ looking for session.json
        ...
        for date_dir in sorted(project_dir.iterdir()):
            ...
            for session_dir in sorted(session_dir for session_dir in date_dir.iterdir() if session_dir.is_dir()):
                ...
                sessions.append({
                    ...
                    "grok_session_id": metadata.get("grok_session_id"),
...
def main():
    parser = argparse.ArgumentParser(description="List past Grok delegation sessions")

...
description: "Plan a feature or task with Codex as co-planner, then execute as PM delegating to subagents. Full workflow: gather requirements, draft plan, iterate with Codex review until consensus, present for user approval, then execute via task delegation with a final Codex code review. Use when the user wants to plan and build something end-to-end with Codex as co-planner and reviewer (Codex does not implement)."
disable-model-invocation: true
---

# Plan-with-Codex: Plan with Codex, Execute as PM
...
3. **Iterative Codex reviews for large plans:** When the plan spans multiple milestones or phases, treat Codex reviews as milestone gates — complete a milestone, run a focused Codex review scoped to that milestone's changes, address all Critical/Major findings, then proceed to the next milestone. This prevents a massive end-of-project review where Codex must digest dozens of files at once, and ensures code written in later phases builds on a reviewed foundation. Start a fresh Codex session for each milestone or phase. Resume a session only for the re-review rounds of that same milestone, and never carry a session across milestones: a resumed session accumulates every earlier prompt and finding, so Codex works with a polluted, noisy context that wastes tokens and degrades review quality. Give each new session the context it needs by telling Codex which files changed in that milestone and what the milestone's goals were. For small plans (single phase or few files), skip milestone gates and do one final review.

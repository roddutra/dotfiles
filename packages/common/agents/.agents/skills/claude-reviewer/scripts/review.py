...
if __name__ == "__main__":
    if len(sys.argv) < 2 or sys.argv[1] not in {"init", "write", "run", "list", "cleanup", "models"}:
        raise SystemExit("usage: review.py {init,write,run,list,cleanup,models} [...]")
    action = {"init": "init-review", "write": "write-prompt", "run": "run-review", "list": "list-reviews", "cleanup": "cleanup", "models": "list-models"}[sys.argv[1]]

#!/usr/bin/env python3
"""List and filter past Grok review sessions.

Walks the directory tree under ~/.grok-reviewer/ looking for session.json
...
def find_round_files(session_dir: Path) -> list[str]:
    ...
    files = sorted(
        ...
        if f.suffix in (".md", ".jsonl") and f.name.startswith("r")
    )
    return files
...
def main():
    parser = argparse.ArgumentParser(description="List past Grok review sessions")

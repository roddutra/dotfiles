#!/usr/bin/env python3
"""Initialize a Grok review session.
...
file. The directory hierarchy encodes project, date, and session identity:
  ...
  ~/.grok-reviewer/<project>/<YYYY-MM-DD>/<HHMMSS-title>/session.json
"""
...
def _resolve_git_root(project_dir: Path) -> Path:
    ...
    in a monorepo, which would cut off Grok's access to files outside it.
    """
    try:
...
def init_session(
    ...
    change Grok's outputs across rounds. To use a different model, start
    a fresh session.
    ...
    # A bare repo has no working tree: Grok cannot review files in it, and we
    ...
    if _is_bare_repo(project_dir):
        print(
            ...
            f"tree): {project_dir}. Grok needs a working tree to review files; "
    ...
    # toplevel (--show-toplevel) so Grok reads the files where you launched.
...
def main():
    parser = argparse.ArgumentParser(description="Initialize a Grok review session")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "also updates the persisted value. If omitted, Grok uses "

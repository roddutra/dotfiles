#!/usr/bin/env python3
"""Clean up review files for a completed Grok review session.
...
def cleanup_session(session_path: Path) -> dict:
    ...
    for f in sorted(session_dir.glob("r*-prompt.md")) + sorted(session_dir.glob("r*-output.md")) + sorted(session_dir.glob("r*-stream.jsonl")):
        f.unlink()
        deleted_count += 1
...
def main():
    parser = argparse.ArgumentParser(description="Clean up Grok review session files")

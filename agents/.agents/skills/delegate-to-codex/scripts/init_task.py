#!/usr/bin/env python3
"""Initialize a Codex delegation session — one session per task.

Creates the session directory and session.json:

  ~/.codex-delegate/<project>/<YYYY-MM-DD>/<HHMMSS-title>/session.json

A session is a single Codex conversation with its own context window. It
...
def init_task(
    ...
    When both are omitted the local Codex CLI defaults apply
    (`~/.codex/config.toml`), so the skill tracks CLI/model upgrades for free.
    """
    ...
    if not _in_git_work_tree(project_dir):
        # Codex refuses to run outside a git repo, and the change tracking
        # (rN-changes.md) needs git anyway.
        print(
            f"Error: {project_dir} is not inside a git work tree. Codex requires a "
            f"git repository; run `git init && git add -A && git commit -m init` first.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Initialize a Codex delegation session (one task per session)")
    ...
    parser.add_argument("--model", default=None, help="Codex model for the whole session (default: local CLI config)")
    parser.add_argument("--reasoning-effort", default=None, help="Initial reasoning effort (default: local CLI config)")
    parser.add_argument("--network", action="store_true", help="Allow network access inside the workspace-write sandbox (package installs, API calls)")
    parser.add_argument("--allow-git", action="store_true", dest="allow_git", help="Let Codex commit/branch (default: git state is off-limits; you own git)")

...
Creates a new session with the same project dir, model, effort, network,
git and verify settings, then copies the old session's briefs, reports and
change summaries into `<project_dir>/.tmp/codex-delegate/<old-session>/` so
the new Codex thread can read them from disk. The new session starts with
...
round-1 brief ("Read .tmp/codex-delegate/<old>/r*-report.md for what was
already done").
"""
...
def _handoff_locked(old_session: Path, title: str) -> dict:
    ...
    handoff_dir = project_dir / ".tmp" / "codex-delegate" / old_dir.name
    ...
    return {
        ...
        "copied": copied,
        "note": "Tell Codex in the round-1 brief to read the copied reports for prior progress; do not inline them.",
    }

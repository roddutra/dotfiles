#!/usr/bin/env python3
"""List and filter past Codex delegation sessions.

Walks the directory tree under ~/.codex-delegate/ looking for session.json
...
from generate_path import DELEGATE_DIR, to_kebab_case
...
def find_round_files(session_dir: Path) -> list[str]:
    """Find all brief/report/changes files in a session directory."""
    files = sorted(
...
def list_sessions(
    ...
    for project_dir in sorted(DELEGATE_DIR.iterdir()):
...
def main():
    parser = argparse.ArgumentParser(description="List past Codex delegation sessions")
